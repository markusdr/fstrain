 ...
 public:
  ...
  MDExpectations& GetMDExpectations() { 
    ...
    if (expectations_->getCount() > 1) {
      MDExpectations* d = new MDExpectations(*expectations_);    // Invoke MDExpectations's copy ctor
  ...
  bool Member() const {
    ...
    bool is_member = Value() == Value()
        && Value() != FloatLimits<double>::kNegInfinity;
    if(!is_member){
      return false;
    }
    ...
    for(MDExpectations::const_iterator it = e.begin(); it != e.end(); ++it){
      ...
      bool is_member = neglog_val == neglog_val
          && neglog_val != FloatLimits<double>::kNegInfinity;
      if(!is_member){
        return false;
      }
    }
  ...
  istream &Read(istream &strm) {
    ...
    ReadType(strm, &value_);
    int64 size0;  
    ReadType(strm, &size0);
    GetMDExpectations().clear();
    MDExpectations::size_type size = (MDExpectations::size_type)size0;
  ...
  ostream &Write(ostream &strm) const {
    WriteType(strm, value_);
    WriteType(strm, (int64)(GetMDExpectations().size()));
    for(MDExpectations::const_iterator it=GetMDExpectations().begin(); it != GetMDExpectations().end(); ++it){
...
inline MDExpectationWeight Plus(const MDExpectationWeight &w1, const MDExpectationWeight &w2) {
  using namespace fstrain::core;
  MDExpectationWeight w3;
  double f1 = w1.Value(), f2 = w2.Value();
  if (f1 == fstrain::core::kPosInfinity)
    w3.SetValue(f2);
  else if (f2 == fstrain::core::kPosInfinity)
    w3.SetValue(f1);
  else if (f1 > f2)
    w3.SetValue(f2 - LogExp(f1 - f2));
  else
    w3.SetValue(f1 - LogExp(f2 - f1));
